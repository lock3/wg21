...
- *Implementability.* Syntax and semantics that cannot be supported by all
  ...
  disqualify seemingly simple or obvious ideas. And we can't force implementers
...
There are two potentially conflicting uses of `^`: Apple's "Blocks"
specification [@blocks] and C++/CLI/CX's managed pointers.
...
always a value, never an function.
